...
def pygmentify_html(text, **kwargs):
    ...
    lexer_names = reduce(lambda a,b: a + b[2], iter(LEXERS.values()), ())
    # custom formatter
    ...
    for pre_match in pre_re.findall(text):
        ...
        work_area = work_area.replace('&nbsp;', ' ').replace('&amp;', '&').replace('&lt;', '<').replace('&gt;', '>').replace('&quot;', '"').replace('&#39;', "'")
        work_area = highlight(work_area, lexer, formatter)
        subs.append([''.join(pre_match), smart_unicode(work_area)])
    for sub in subs:
